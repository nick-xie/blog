...
We live in a world with lots of engineered sound. Specifically in music, with technologies such as MIDI and workstations like Ableton and Melodyne, it has become the norm to listen to fully digitally synthesized sounds of acoustic instruments (or at least feature very minimal sampling of live recordings). For some, this might be a bit artistically unsettling.

In my computational audio course at school, as an introduction to the concept of timbre, the professor rhetorically asked "why does the same note sound different played across different instruments". Although I've played music nearly my entire life, I was surprised to realize that I had never really asked myself this. Connecting this idea to the abundance of digitally synthesized acoustic sounds, I wanted to make a project to explored this topic. The mission of the project is to synthesize instrument sounds derived from real audio input. The goal is to take a recording of a real instrument and synthesize it into sounding like another instrument. While the resulting audio is still digitally made, in some sense there would still a bit of “real” instrument audio left in it, making it more “alive”.
...
Sound is generally thought to be a wave. Some people might think of a sin wave when thinking about how sound is digitally stored. Mathematically, a sin wave is continuous meaning that it's expressed by some function and that it has infinite amount of points on it. Digitally however, sound is stored as a set of *discrete* points. This means that while yes, sound could be a sin wave, instead of storing the mathematical function of sin, it is digitally stored as a bunch of individual points that if you played connect the dots with, would form a sin curve. Each of these individual points are called *samples* and it stores a value that is used to map it on a time-amplitude domain. Samples are evenly spaced horizontally and the density of them is determined by the audio bitrate which is usually 44100. This means that there are 44100 samples in one second. So in just one second of audio, there are 44100 points which can be plotted to form the "curve" of the sound wave.

In terms of the amplitude, for the sake of this project discussion, I normalized everything between 0.7 and -0.7 meaning that all samples have a value between 0.7 and -0.7. I did this because after a few failed experiments at the start, I nearly blew my eardrums out when listening to something that got scaled up to the 100's in terms of amplitude.

# Setting the scene
To frame the scope of the project. I picked two instruments that I wanted to synthesize from one to another. I ended up picking the violin and flute for a few reasons. A lot of an instrument’s unique sound (aka timbre) is defined by its particular musical envelope. This musical envelope is determined by a lot of things such as overtones as well as the physical mechanics of the particular instrument. All together, it forms the "shape" of the waveform. The flute and violin's respective musical envelopes are already fairly similar which will make the synthesis from one to the other a little bit easier. As well, neither instrument' envelopes contain any particularly distinctive elements that are instrument specific and deviate out of the generic musical envelope model.

I used the [Musical Instrument Database by the University of Iowa](http://theremin.music.uiowa.edu/MIS.html) to obtain clean recordings of a flute and violin playing a C5 note for about 1-2 seconds each. I loaded up a blank Python (Jupyter) notebook and I was all set to begin writing software that would take the two recordings and produce a flute sound synthesized from the violin recording. Hopefully, the techique could be applied universally for all violin notes (without needing a different flute recording for each one).
...
My first approach was fairly straightforward and had moderate success. I would pass in the source and destination recordings to come up with a **scaleFactor** array. The scaleFactor array kept track of the ratio between each sample value between the source and destination. For example, at the 800th sample, if the source had a value of 0.25 and the destination had a value of 0.50, then the scaleFactor array at 800 would store the value of 2.0 since that's what the source sample would need to be multiplied by to match the destination sample.
...
When I first listened to the first synthesis above, I nearly jumped out of my chair. It wasn't the garbled mess I was expecting, it actually sounded like a flute/woodwind, it worked! Or did it...?
...
Introduce the Attack, Decay, Sustain, and Release (ADSR) model. Recall that I mentioned that I purposefully chose the violin and flute due to their similar musical envelope. A musical envelope is usually categorized into these four phases of a note's lifespan. Each phase has a general repeating waveform and all combined, it makes up the total note. I also noticed that each characteristic wave of each phase of the note remained largely unchanged across different notes of the same instrument.

{% include image.html url="../../../assets/synthesis/adsr.jpg" height="15rem" description="Not my diagram, shoutout to lynda.com" %}

I figured that if I was able to find a universal transform function from a violin sustain waveform to a flute sustain waveform and then smartly apply this transformation across the entire sustain of the violin, then I would find some success. If that seemed to work, I could do the same for the attack, decay, and release and end up with a more complicated but more robust procedure. I chose to start with the sustain since it is the longest part of the note.
...
For now, I admit I am stumped on how to proceed. I feel like the main issue lies in finding an S that works for V as well as the (wide) range of variations, V'. The wave finding algorithm is definitely not perfect but I think it's alright enough so that if I had a better S, the output would sound much closer to a flute than what I got out.
